...
append =
  alternative
    [ appendTo BaseName . mpOption autonum . mconcat $
        [ long "append-autonum"
        , metavar "[START]/[INCREMENT]/[PAD]"
        , help "Append an autonumber. See --prepend options for details."
        ]
    , appendTo Extension . mpOption autonum . mconcat $
        [ long "append-autonum-ext"
        , metavar "[START]/[INCREMENT]/[PAD]"
        , internal
        ]
    , appendTo FullName . mpOption autonum . mconcat $
...
prepend =
  alternative
    [ prependTo BaseName . mpOption autonum . mconcat $
    ...
    , prependTo Extension . mpOption autonum . mconcat $
        [ long "prepend-autonum-ext"
        , metavar "[START]/[INCREMENT]/[PAD]"
        , internal
        ]
    , prependTo FullName . mpOption autonum . mconcat $
...
lowercase =
  alternative
    [ flag' (BaseName Lowercase) . mconcat $
        [ long "lowercase"
        , help "Convert to lowercase"
        ]
    , flag' (Extension Lowercase) . mconcat $
        [ long "lowercase-ext"
        , internal
        ]
    , flag' (FullName Lowercase) . mconcat $
        [ long "lowercase-full"
        , internal
        ]
    ]
...
uppercase =
  alternative
    [ flag' (BaseName Uppercase) . mconcat $
        [ long "uppercase"
        , help "Convert to uppercase"
        ]
    , flag' (Extension Uppercase) . mconcat $
        [ long "uppercase-ext"
        , internal
        ]
    , flag' (BaseName Uppercase) . mconcat $
...
setTo =
  alternative
    [ fmap (BaseName . SetTo) . strOption . mconcat $
        [ long "set"
        , metavar "VALUE"
        , help "Set the name to a given value"
        ]
    , fmap (Extension . SetTo) . strOption . mconcat $
        [ long "set-ext"
        , internal
        ]
    , fmap (FullName . SetTo) . strOption . mconcat $
        [ long "set-full"
        , internal
        ]
    ]
...
regex =
  alternative
    [ fmap (construct BaseName) . mpOption sed . mconcat $
        [ long "regex"
    ...
    , fmap (construct Extension) . mpOption sed . mconcat $
        [ long "regex-ext"
        , internal
        ]
    , fmap (construct FullName) . mpOption sed . mconcat $
        [ long "regex-full"
        , internal
        ]
    ]
...
subdir =
  fmap SubDir . OA.option relDirReader . mconcat $
    [ long "subdir"
    , metavar "DIR"
 ...
 where
  relDirReader =
    maybeReader $
      fmap Path.dirFromFile . Path.fileFromDir . Path.path
